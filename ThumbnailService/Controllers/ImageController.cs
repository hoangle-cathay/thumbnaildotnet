        ...
        public async Task<IActionResult> List()
        {
            ...
            var images = await _db.ImageUploads.Where(i => i.UserId == userId)
                ...
                .Select(i => new {
                    ...
                    i.UploadedAtUtc,
                    i.ThumbnailStatus,
                    ThumbnailUrl = i.ThumbnailStatus == ThumbnailStatus.Completed && !string.IsNullOrEmpty(i.ThumbnailGcsPath) 
                        ? _storage.GetPublicUrl(_thumbnailsBucket, i.ThumbnailGcsPath) 
                        : null
                })
                .ToListAsync();
